 ...
 */
class UniversalDataExtractionController extends ControllerBase {
  ...
  public function dataExtraction() {
    ...
    $node_ids = $this->entityTypeManager->getStorage('node')
      ->getQuery()
      ->condition('type', 'aliased_content')
      ->accessCheck(TRUE)
      ->execute();
    ...
    $html_output .= '<p>Universal page Data extraction<p>';
    ...
    foreach ($node_ids as $node_id) {
      ...
      if ($node->hasField('bhcc_components')) {

        $page_sections = $node->get('bhcc_components')->getValue();
